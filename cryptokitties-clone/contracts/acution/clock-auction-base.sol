...
contract ClockAuctionBase is ERC721Holder {
    ...
    modifier canBeStoredWith64Bits(uint256 _value) {
        //// revert 메세지 추가
        require(
            _value <= 18446744073709551615,
            "_value exceeds 128 bits limmit"
        );
        _;
    }
    ...
    function _transfer(address _receiver, uint256 _tokenId) internal {
        ...
        //// transfer()가 더이상 표준이 아니기 때문에 safeTransferFrom으로 변경
    ...
    /// @dev Adds an auction to the list of open a uctions. Also fires the
    ...
    function _bid(
        ...
        //// revert 메세지 추가
        require(_isOnAuction(auction), "this _tokenId is not on auction");

        // Check that the incoming bid is higher than the current
        // price
        uint256 price = _currentPrice(auction);
        //// revert 메세지 추가
        require(_bidAmount >= price, "bid amount must be >= price");
        ...
        if (price > 0) {
            ...
            //// seller에게 판매금액에서 수수료를 뺀 금액을 receive()를 call하여 전송
            (bool succeess, ) = payable(seller).call{value: sellerProceeds}("");
            require(succeess, "Failed to send sellerProceeds to seller");
        }
